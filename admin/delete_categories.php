...
if(!isset($_POST['delete_categorie']) && !empty($_GET['idc']) && !empty($_GET['categorie'])) {
	$query = "select id, name from categories where id =".$_GET['idc'];
	...
	$smarty_delete_categories->assign("name", $next['name']);

	$content = $smarty_delete_categories->fetch("delete_categories.tpl");
}


else if(isset($_POST['delete_categorie']) && !empty($_POST['idc']) && !empty($_POST['name']) && $_POST['del'] == "yes") {
	$query_delete = "delete from categories where id =".$_POST['idc'];
	mysqli_query($dbc, $query_delete) or die("Error query_delete");

	$content = "Категория ".$_POST['name']." успешно удалена<br /><a href='view_categories.php'>Вернуться к списку категорий</a>";
}

else {
	$content = "Недостаточно данных для редактирования<br /><a href='delete_categories.php?idc=".$_POST['idc']."&categorie=".$_POST['name']."'>Попробовать снова</a>";
}
