...
if(!isset($_POST['add_categorie'])) {
	$content = $smarty_add_categories->fetch("add_categories.tpl");
}
	...
	if($_FILES['photo']['error'] == 0){
		...
		$query = "insert into categories (name, photo, exist) values('".$_POST['name']."', '$photo_name','".$_POST['exist']."')";
	}

	else {
		$query = "insert into categories (name, exist) values('".$_POST['name']."', '".$_POST['exist']."')";
	}

	/*echo $query;*/
	mysqli_query($dbc, $query) or die("Error query");

	$content = "Категория успешно добавлена<br /><a href='view_categories.php'>Вернуться к списку категорий</a><br /><a href='add_categories.php'>Добавить еще категорию</a>";
