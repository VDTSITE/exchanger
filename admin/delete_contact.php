...
if(isset($_SESSION['user_id']) && isset($_SESSION['user_name']) && isset($_SESSION['user_rights'])) {
		...
		$smarty_delete_contact = new Smarty();

	if(isset($_GET['idc'])) {
		

		$query_delete_mail_idc = "select id, name from contact where id =".$_GET['idc'];

		//echo $query_delete_mail_idc;
		...
		$smarty_delete_contact->assign("id", $next_delete_mail_idc['id']);
		$smarty_delete_contact->assign("name", $next_delete_mail_idc['name']);
		$content = $smarty_delete_contact->fetch("delete_contact.tpl");
	}
		...
		$content = "Сообщение клиента ".$next_name['name']." успешно удалено.<br /><a href='view_contact.php'>Вернуться к списку сообщений</a>";
		
	}

	else {
		$content = "Недостаточно данных для удаления.<br /><a href='delete_contact.php?idc=".$_POST['id']."'>Попробуйте снова</a>";
	}
