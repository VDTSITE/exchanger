...
if(empty($_FILES['photo_slider1'])) {
	$content = $smarty_add_slider->fetch("add_slider.tpl");
}
	...
	if($_FILES['photo_slider1']['error'] == 0) {
		...
		$query_slider = "insert into main_slider (photo, title) values('$photo_name', '".$_POST['title']."')";
		
	}

	else {

		$query_slider = "insert into main_slider (title) values('".$_POST['title']."')";
	}
	...
	$content = "Данный слайдер успешно добавлен<br /><a href='view_slider.php'>К списку слайдеров</a>";
