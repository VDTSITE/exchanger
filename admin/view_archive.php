...
$smarty_view_archive = new Smarty();


$mark = 5;
$smarty_view_archive->assign("mark", $mark);

$query = "select clients.id as clients_id, name, email, tel, comment, count_tovarov, rel_zakaz.data, products.id as products_id from clients inner join rel_zakaz on clients.id = rel_zakaz.id_client inner join products on rel_zakaz.id_tovara = products.id where rel_zakaz.status = 1 order by data desc, id_client desc";
...
$count_count = 1;
$schetchik = 1;
$smarty_view_archive->assign("schetchik", $schetchik);
...
$smarty_view_archive->assign("order", $order);

$content = $smarty_view_archive->fetch("view_orders.tpl");
