...
if(!isset($_POST['edit_categorie']) && !empty($_GET['idc'])) {
	$query = "select id, photo from nails_slider where id =".$_GET['idc'];
	...
	$content = $smarty_edit_categorie->fetch("edit_nails_slider.tpl");
}
	...
	if($_FILES['new_photo']['error'] == 0){
		...
		$query_edit = "update nails_slider set photo='$photo_name' where id=".$_POST['idc'];
		
		mysqli_query($dbc, $query_edit) or die("Error query_edit");

		$content = "Вы успешно отредактировали слайд<br /><a href='view_nails_slider.php'>Вернуться к списку слайдов</a>";
	}
}
