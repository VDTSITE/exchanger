...
if(isset($_SESSION['user_id']) && isset($_SESSION['user_name']) && isset($_SESSION['user_rights'])) {
	...
	if(isset($_GET['idc'])) {
		...
		$content = $smarty_delete_mail_idc->fetch("delete_slider.tpl");
	}


	else if(!empty($_POST['id']) && !empty($_POST['delete']) && $_POST['del'] == "yes") {

		$query = "delete from main_slider where id=".$_POST['id'];

		//echo $query;

		mysqli_query($dbc, $query) or die("Error query");

		$content = "Данный слайд успешно удалён.<br /><a href='view_slider.php'>Вернуться к списку слайдов</a>";
		
	}

	else {
		$content = "Недостаточно данных для удаления слайда.<br /><a href='delete_slider.php?idc=".$_POST['id']."'>Попробуйте снова</a>";
	}
