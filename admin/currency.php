...
if(isset($_POST['select_currency'])) {
	...
	$query_status_null = "update currency set status=0";
	$result_status_null = mysqli_query($dbc, $query_status_null) or die("Error query_status_null");

	$query_status_main = "update currency set status=1 where id=".$_POST['select_currency'];
...
if(isset($_GET['act_currency'])) {
	if($_GET['act_currency'] == 1) {
		$query_act = "update currency set act=1 where id=".$_GET['ida'];
	}

	else {
		$query_act = "update currency set act=0 where id=".$_GET['ida'];
	}
...
/*	$query_num = "select id from currency";
	...
	$query_active_currency = "select buy, sell from currency where status = 1";
	...
	$query = "select id, name, sell, buy, status, act from currency";
