...
$query_mail = "select id, name, tel from contest order by data desc";
/*echo $query_mail;*/
...
while($next_mail = mysqli_fetch_array($result_mail)) {
	$mail[] = array("id"=>$next_mail['id'], "name"=>$next_mail['name'], "tel"=>$next_mail['tel'], "date"=>$next_mail['date']);
}
...
$content = $smarty_mail->fetch("contest.tpl");
