...
<input type="number"  name="ammount_refill" id="ammount_refill1" placeholder="0.00">
            ...
            {section loop=$currency_wm name=i}
              <option value="{$currency_wm[i].buy}"><span id="currency_name_refill1">{$currency_wm[i].name}</span></option>
            {/section}
            </select>
...
<input class="select_exchangeinput" type="number" id="k_oplate_refill1" placeholder="0.00" style="margin: 0px;">
              ...
              {section loop=$currency name=i}
                <option id="myOption" value="{$currency[i].buy}" label="{$currency[i].name}">{$currency[i].name}</option>

              {/section}
			   {section loop=$currency name=i}
              <input type="hidden" class="perebor" id="{$currency[i].name}" value="{$currency[i].buy}">
...
<input type="number"  name="ammount_cashing" id="ammount_cashing1" placeholder="0.00">
              ...
              {section loop=$currency name=i}
                <option id="myOption_cashing" value="{$currency[i].buy}" label="{$currency[i].name}">{$currency[i].name}</option>
...
<h4>К оплате</h4>
            ...
            {section loop=$currency_wm name=i}
              <option value="{$currency_wm[i].buy}"><span id="currency_name_cashing1">{$currency_wm[i].name}</span></option>
            {/section}
            </select>
   ...
   padding: 2px;
    ...
    <tbody>
      {section loop=$categories name=i}
		...
		{else}
			{if $categories[i].type_operation == 0}
        ...
        <td>{if $categories[i].type_operation == 0}Ввод{else}Вывод{/if}</td>
		    ...
		    <td>{section loop=$currency_wm name=l}{if $currency_wm[l].id == $categories[i].id_currency}{$currency_wm[l].name}{/if}{/section}</td>


        <td>{$categories[i].percent} %</td>
        <td>{$categories[i].k_oplate}</td>
		  <td>{$categories[i].k_polucheniju}</td>
		  <td>{if $categories[i].actual == 0}{literal}

							<span id="span_send_stock" onclick="send_dialog_stock({/literal}{$categories[i].id}{literal})">
							...
							{/literal}{else}<b>Оформить заявку</b>{/if}</td>
...
<div class="one-quarter animate_afr  vc_custom_1437646393203 color_custom animate_start" style="padding: 1%; vertical-align: baseline;"><div class="wpb_text_column "><div class="wpb_wrapper">
    ...
    {section loop=$currency1 name=i}
    <tr>
    	{if $currency1[i].name != "UAH"}
        <th scope="row">{$currency1[i].name}</th>
        <td style="background: rgb(243, 255, 228) none repeat scroll 0% 0%;">{$currency1[i].buy}</td>
		<td style="background: rgb(255, 245, 245) none repeat scroll 0% 0%;">{$currency1[i].sell}</td>
		{/if}
	</tr>
	{/section}
...
<div id="text-7" class="widget widget_text"><h4>Shilling.co</h4> <div class="textwidget">Сервис Shilling.co - это площадка для брокерских
	...
	Сервис так же предоставляет возможность получения или обмена титульных
...
<script type='text/javascript' src='/js/jquery/jquery.themepunch.tools.min.js' defer='defer'></script>
<script type='text/javascript' src='/js/jquery/jquery.themepunch.revolution.min.js' defer='defer'></script>
...
<script type='text/javascript' src='/js/us.core.js'></script>
<script type='text/javascript' src='/js/us.widgets.js'></script>
<script type='text/javascript' src='/js/us.theme.js'></script>
